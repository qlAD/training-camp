    ...
    <div className={`w-full ${className}`}>
      {finished ? (
        <motion.div
          ...
          className="mx-auto w-full max-w-md rounded-2xl border border-teal-400/40 bg-teal-400/10 px-6 py-8 text-center"
        >
          <p className="font-mono text-5xl font-black text-white">
            {score}
            <span className="text-2xl text-teal-300">/{questions.length}</span>
          </p>
          <p className="mt-2 text-sm font-bold text-teal-100">
            {score === questions.length ? '全对！今天真的懂了 🎉' : '答对一部分，回去复习对应的镜头'}
          </p>
        </motion.div>
      ) : (
