...
const easeOut = { duration: 0.4, ease: EASE };
...
export const Checkoff: React.FC<CheckoffProps> = ({ at, items, className = '' }) => {
  ...
  return (
    <div className={`w-full ${className}`}>
      <div className="mx-auto w-full max-w-md space-y-2">
        {items.map((item, i) => {
          ...
          return (
            ...
            >
              <span
                className={`flex h-5 w-5 shrink-0 items-center justify-center rounded-full border text-[11px] font-bold transition-colors duration-300 ${
      ...
      <motion.p
        initial={{ opacity: 0 }}
        animate={{ opacity: s(items.length) ? 1 : 0 }}
        transition={easeOut}
        className="mt-3 text-center text-sm font-bold text-lime-300"
      >
        ✅ 今日打卡目标：100%
      </motion.p>
    </div>
