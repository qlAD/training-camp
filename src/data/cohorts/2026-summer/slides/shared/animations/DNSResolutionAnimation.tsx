...
import { Play, Pause, RotateCcw, ChevronLeft, ChevronRight, Globe, Server, Network, Search, Target } from 'lucide-react';

type DnsNode = 'browser' | 'dns' | 'root' | 'tld' | 'authoritative' | 'target';
...
interface DNSResolutionAnimationProps {
  steps: DnsStep[];
  /** 可覆盖各节点显示名，缺省使用技术角色名 */
  nodeLabels?: Partial<Record<DnsNode, string>>;
}

const ORDER: DnsNode[] = ['browser', 'dns', 'root', 'tld', 'authoritative', 'target'];

const DEFAULT_LABELS: Record<DnsNode, string> = {
  browser: '浏览器',
  dns: '本地 DNS',
  root: '根 DNS',
  tld: '顶级域 DNS',
  authoritative: '权威 DNS',
  target: '目标 IP',
};

const NODE_ICON: Record<DnsNode, React.ComponentType<{ className?: string }>> = {
  browser: Globe,
  dns: Server,
  root: Network,
  tld: Server,
...
export const DNSResolutionAnimation: React.FC<DNSResolutionAnimationProps> = ({ steps, nodeLabels }) => {
  ...
  return (
    <div className="w-full">
      <style>{`
        @keyframes dns-pulse { 0%,100%{ transform: scale(1);} 50%{ transform: scale(1.08);} }
        @keyframes dns-flow { to { stroke-dashoffset: -14; } }
      `}</style>
      ...
      <div className="rounded-2xl bg-slate-950/60 border border-slate-700/60 p-5">
            ...
            return (
              <React.Fragment key={key}>
                <div
                  ...
                  style={{ animation: isCurrent ? 'dns-pulse 1.3s ease-in-out infinite' : undefined }}
                >
                  <div
                    className={`h-10 w-10 rounded-lg flex items-center justify-center mb-1.5 border transition-colors ${
                      isCurrent
                  ...
                  <span className={`text-[11px] font-bold ${isCurrent ? 'text-white' : isDone ? 'text-emerald-300' : 'text-slate-500'}`}>
                    {labels[key]}
                  </span>
                </div>
                {i < ORDER.length - 1 && (
                  <div className="flex items-center px-0.5">
                    <svg width="36" height="20" viewBox="0 0 36 20" className="overflow-visible">
                      <line
                        ...
                        strokeWidth="2"
                        strokeDasharray="5 4"
                        className={visited.has(ORDER[i]) && visited.has(ORDER[i + 1]) ? 'stroke-emerald-400' : 'stroke-slate-600'}
                        style={{
                          animation: visited.has(ORDER[i]) && visited.has(ORDER[i + 1]) ? 'dns-flow 0.8s linear infinite' : undefined,
                        }}
                      />
                      <polygon
                        points="28,4 34,10 28,16"
                        className={visited.has(ORDER[i]) && visited.has(ORDER[i + 1]) ? 'fill-emerald-400' : 'fill-slate-600'}
                      />
                    </svg>
                  </div>
                )}
        ...
        <div className="mt-4 text-center">
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md bg-amber-500/15 border border-amber-500/30 text-amber-200 text-xs font-bold">
      ...
      <div className="flex items-center justify-between gap-2 pt-3 mt-3 border-t border-slate-700/60">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button onClick={toggle} className="px-3 py-1.5 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-medium flex items-center gap-1">
